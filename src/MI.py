#!/usr/bin/env python
# mi_rank_eegnet_curve.py  ---  accuracy vs #channels using MI ranking
# ----------------------------------------------------------------------
...
for matf in FILES:
    ...
    # ------------- 互信息打分 -------------
    # 对每个 trial 计算每通道 log-variance 作为特征 (N_trial, 60)
    logvar = np.log(trials.var(axis=-1) + 1e-10)
    mi_scores = mutual_info_classif(logvar, labels, random_state=SEED)
    order = np.argsort(mi_scores)[::-1]          # MI 高 → 前排

    for K in K_LIST:
        sel = order[:K]
        selected_names = [CHAN_NAMES[i] for i in sel]
        print(f"Top-{K} MI channels: {sel.tolist()} ({selected_names})")
...
# -------- 输出结果 --------
print("\n#Channels | acc_mean ± std (MI ranking)")
print("---------------------------------------")
for k in K_LIST:
