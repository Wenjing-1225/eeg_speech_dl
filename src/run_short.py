...
# -*- coding: utf-8 -*-
"""
run_neuroxai_eegnet_short.py  ——  Short-words  (3-class: in / out / up)
...
BATCH      = 128
THR_BASE   = .60
FBCSP_BANDS = [(4+i*4, 8+i*4) for i in range(8)]
CANDIDATE   = 30
N_CLASS     = 3          # ★修改：三分类
...
def fast_ica_all(trials, n_comp=None):
    ...
    ica = FastICA(n_components=C if n_comp is None else n_comp,
                  whiten='unit-variance', random_state=SEED, max_iter=300)
...
def fbcsp_rank(trials, labels):
    ...
    for c in classes:                       # one-vs-rest
        ...
        for low, high in FBCSP_BANDS:
            ...
            csp.fit(fb.astype(np.float64), y_bin)
            w   = csp.filters_
...
class EEGNet(nn.Module):
    def __init__(self, C, n_cls=N_CLASS, dropout=0.25):
        super().__init__()
...
def train_net(X,y,epochs,lr=1e-3,dropout=0.25):
    ...
    for _ in range(epochs):
        ...
        for beg in range(0,len(idx),BATCH):
            sl = idx[beg:beg+BATCH]; opt.zero_grad()
...
def neuroxai_imp(base,trials,labels,cand_idx,n_samples):
    ...
    brain = BrainExplainer(25, ['in','out','up'])   # ★修改：三类标签
    gexp  = GlobalBrainExplainer(brain)
    gexp.explain_instance(x=trials[:,:,cand_idx],
                          y=labels,
                          classifier_fn=clf,
...
def main(k_list,n_samples,use_ica=True):
    ...
    for subj_i,matf in enumerate(FILES,1):
        ...
        if use_ica: trials = fast_ica_all(trials)
        ...
        Xw = np.ascontiguousarray(np.stack(Xw))
        ...
        def cv_subset(sel_idx,epochs):
            idx_t = torch.from_numpy(np.asarray(sel_idx,dtype=np.int64).copy()).to(DEVICE)
            Xi = X_t.index_select(2,idx_t)
            scores=[]
        ...
        acc30,std30 = cv_subset(cand_idx,EPOCH_BASE)
